...
public class MediaCodecDecoderRenderer extends VideoDecoderRenderer {
    ...
    private long nativeVideoDecoder;
    ...
    public int setup(int format, int width, int height, int redrawRate) {
        ...
        if (use_ndk) {
            ...
            MoonBridge.setBufferCount(nativeVideoDecoder, prefs.bufferCount);
                ...
                public void run() {
                    ...
                    while (!stopping) {
                        try {
                            ...
                            // 使用官方渲染逻辑（我的逻辑在不可控的java端可能表现不好）
                            int outIndex = videoDecoder.dequeueOutputBuffer(info, usTimeout);
                            if (outIndex >= 0) {
    ...
    public void prepareForStop() {
        ...
        if (nativeVideoDecoder != 0) {
            ...
            MoonBridge.getVideoStats(nativeVideoDecoder, globalVideoStats);

            MoonBridge.stopMediaCodec(nativeVideoDecoder);
        }
    }
    ...
    public void cleanup() {
        ...
        if (nativeVideoDecoder != 0)
            MoonBridge.deleteMediaCodec(nativeVideoDecoder);
